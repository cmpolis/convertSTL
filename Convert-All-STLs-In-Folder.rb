#!/usr/bin/env ruby
# Convert-All-STLs-In-Folder.rb - Converts all STL files in folder between binary and ASCII encoding
# by IsRightKev & Chris Polis
#
...
# 
# USAGE: 
# (1) copy script to folder with the stl files to convert 
# (2) $ cd folder
# (3) $ ruby convertSTL.rb 
...
begin
# Get all stl's in folder
Dir.glob("*.stl") {|filename|
...
begin
  ...
  if tempLine.start_with? "solid"
    outFilename = "#{filename}".sub(/\.stl/i, '-binary.stl')

    puts "#{filename} is in ASCII format, converting to BINARY: #{outFilename}"
	...
	outFilename = "#{filename}".sub(/\.stl/i, '-ascii.stl')
    puts "#{filename} is in BINARY format, converting to ASCII: #{outFilename}"
    outFile = File.new(outFilename, "w")
    outFile.write("solid \n")
